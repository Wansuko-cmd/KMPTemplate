...
inline fun <T, E, NE> ApiResult<T, E>.mapFailure(block: (E) -> NE): ApiResult<T, NE> = when (this) {
    is ApiResult.Success -> this
    is ApiResult.Failure -> ApiResult.Failure(block(value))
}

inline fun <T, E> ApiResult<T, E>.consume(success: (T) -> Unit = {}, failure: (E) -> Unit = {}) {
    when (this) {
...
inline fun <T, E> ApiResult<T, E>.onEach(block: (T) -> Unit) = this.map {
    block(it)
    it
}
